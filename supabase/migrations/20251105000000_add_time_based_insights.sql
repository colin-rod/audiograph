...
AS $$
  SELECT
    ...
    date_trunc('week', ts) + INTERVAL '6 days' AS week_end,
    EXTRACT(ISOWEEK FROM ts)::int AS week_number,
    EXTRACT(ISOYEAR FROM ts)::int AS year,
...
AS $$
  WITH distinct_days AS (
    SELECT DISTINCT (ts AT TIME ZONE 'UTC')::date AS listen_day
    FROM listens
    WHERE
      ts IS NOT NULL
      AND ms_played IS NOT NULL
  ...
  SELECT
    COALESCE(longest.streak_length, 0) AS longest_streak,
    longest.streak_start,
    longest.streak_end,
    COALESCE(current.streak_length, 0) AS current_streak,
